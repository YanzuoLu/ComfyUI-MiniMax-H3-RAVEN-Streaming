...
| **M5** | Release preparation: docs carrying measured behaviour, `.comfyignore` verified against a package dry run, manual Comfy Registry publish under `yanzuolu` only after approval. | **local preparation done.** Metadata validates; `comfy node pack` produced a 44-file archive whose allow/exclusion audit passed (`validation.md` §Packaging); a reviewable local history exists. **Publication remains intentionally undone:** no public remote, push, tag, release or Registry entry. |
