...
| **M5** | Release preparation: docs carrying measured behaviour, `.comfyignore` verified against a package dry run, manual Comfy Registry publish under `oliveryanzuolu` only after approval. | **source repository published.** `main` is public at `YanzuoLu/ComfyUI-MiniMax-H3-RAVEN-Streaming`; metadata validates; `comfy node pack` produced a 45-file archive whose allow/exclusion audit passed (`validation.md` §Packaging). **Still intentionally undone:** tag and GitHub Release. |
